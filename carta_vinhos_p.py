...
def main():
    ...
    ag_columns = ["selecionado","foto","cod","descricao","pais","regiao","preco_base","preco_de_venda","fator","idx"]
    gb = GridOptionsBuilder.from_dataframe(view_df[ag_columns])
    gb.configure_default_column(editable=True, filter=True, resizable=True, sortable=True)
    gb.configure_column("selecionado", header_name="Selecionado", editable=True, cellEditor="agCheckboxCellEditor", width=120)
    gb.configure_column("foto", header_name="Foto", editable=False, width=70)
    gb.configure_column("cod", header_name="COD", editable=False, width=100)
    gb.configure_column("descricao", header_name="DESCRICAO", editable=False, width=320)
    gb.configure_column("pais", header_name="PAIS", editable=False, width=140)
    gb.configure_column("regiao", header_name="REGIAO", editable=False, width=160)
    gb.configure_column("preco_base", header_name="PRECO_BASE", type=["numericColumn"], valueFormatter="x != null ? `R$ ${Number(x).toFixed(2)}` : ''", width=130)
